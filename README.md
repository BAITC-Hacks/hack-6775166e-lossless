...
Нужен **Python 3.10+** и современный браузер. Числовой расчёт, интерфейс и сравнение планов используют стандартную библиотеку Python; `pip install` и сетевой доступ для основного сценария не нужны. Предыдущий интеграционный кандидат проверен на Python 3.12.1; результаты и границы проверки указаны ниже. Для ответа внешней AI-модели нужен доступ к выбранному провайдеру; NVIDIA дополнительно требует Python SDK `openai`.
...
После объединения с `main` (`63dc040`) 23 сентября: **65 тестов прошли за 37,131 с**; синтаксис JavaScript и `git diff --check` проверены. Через интерфейс быстрый проверочный сценарий дал **Score 56,54**, стоимость **95** и настоящий ответ OpenAI. Проверка чистой копии финального кандидата выполняется отдельно. Протокол — в [журнале AI-интеграции](docs/model-run-issue.md).
...
## Протокол предыдущего интеграционного кандидата

23 сентября 2026: чистая копия отслеживаемых файлов коммита `58d5c11` без `.env` и `.venv` запущена по командам выше на Python 3.12.1. **47 тестов прошли**, проверены оптимизатор, применение плана, 3D-состояние города, резервный режим и отображение ошибок. В этом кандидате также подтверждены живые OpenAI-объяснения всех трёх сценариев на `gpt-5.6-sol`. Новый советник по приоритетам, быстрый пример и перетаскивание мер добавлены последующим объединением с `main`; их финальная проверка проводится отдельно. Подробности и ограничения — в [протоколе проверки](docs/model-run-issue.md).
