...
Проверяйте живую модель в интерфейсе, запущенном из **того же клона, где лежит `.env`**. На другом сервере или в worktree без этого файла советник покажет расчётные факты без вызова LLM. После изменения `.env` перезапустите сервер и обновите страницу. Метка **«AI-разбор проверенных расчётов»** означает ответ модели; **«Расчётные факты · AI-модель не настроена/недоступна»** означает резервный разбор. Три числовых варианта всегда считает симулятор.
