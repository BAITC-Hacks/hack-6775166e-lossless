...
Нужен **Python 3.10+** и современный браузер. Числовой расчёт, интерфейс и сравнение планов используют стандартную библиотеку Python; `pip install` и сетевой доступ для основного сценария не нужны. Интегрированный кандидат проверен на Python 3.12.1. Для ответа внешней AI-модели нужен доступ к выбранному провайдеру; NVIDIA дополнительно требует Python SDK `openai`.
...
При отсутствии ключа или сбое API приложение показывает объяснение из вычисленных фактов с соответствующей пометкой. Основной сценарий доступен эксперту без личного ключа участника. На объединённом кандидате через интерфейс подтверждены настоящие OpenAI-ответы `gpt-5.6-sol` для контрольного плана, одной замены и глобального оптимума. Проверка запуска из чистой копии и отправленного commit выполняется отдельно перед сдачей. Протокол и команды — в [журнале AI-интеграции](docs/model-run-issue.md), более ранняя проверка `gpt-4.1-mini` — в [историческом протоколе](docs/model-verification.md). Успешный сквозной NVIDIA-прогон пока не подтверждён.
